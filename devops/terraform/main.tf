...
module "users_data_table" {
    source = "./modules/dynamodb"
    db_table_name = "${var.service_name}-${var.stage}-users-data-table"
    hash_key = "uid"
    range_key = "role"
    hash_key_type = "S"
    range_key_type = "S"
...
module "chat_data_table" {
    source = "./modules/dynamodb"
    db_table_name = "${var.service_name}-${var.stage}-chat-data-table"
    hash_key = "roomId"
...
module "private_blockchain" {
    source =  "./modules/ec2"
    name = "${var.service_name}-${var.stage}-private-blockchain"
    ...
    chain_network_id = var.chain_network_id
    chain_account_password = var.chain_account_password
}
...
module "secrets_manager" {
    ...
    users_table_id = module.users_data_table.id
    chat_table_id = module.chat_data_table.id
    private_blockchain_ip = module.private_blockchain.public_ip
}
