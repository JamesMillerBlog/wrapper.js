...
export class Template implements TemplateInterface {
  static templateName = "WebXR";
  name = Template.templateName;
  config: WebxrConfiguration | undefined;
  secrets: CreateWebxrSecrets | undefined;
  ...
  createSecrets = (config: WebxrConfiguration): CreateWebxrSecrets => {
    const {
      stage,
      region,
      root_domain_name,
  ...
  requestConfiguration = async (): Promise<WebxrConfiguration> => {
    ...
    const { ready_player_me }: { ready_player_me: string } =
      await inquirer.prompt(ready_player_me_prompt);
