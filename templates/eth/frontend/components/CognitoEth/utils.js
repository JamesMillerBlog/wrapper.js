...
const getUserNonce = (address) => axios({
    method: 'post',
...
const login = (nonce, signature, address) => axios({
    method: 'post',
    ...
    if (window.ethereum) {
      try {
        ...
        if(AccessKeyId && SecretKey && SessionToken && Expiration) {
            authenticateUser(setCognito, setSignInState, address, AccessKeyId, SecretKey, SessionToken, Expiration);
        }
...
const authenticateUser = (setCognito, setSignInState, address, accessKeyId, secretAccessKey, sessionToken, expiration) => {
...
export const checkCredentials = (setCognito, setSignInState) => {
    ...
    const secretAccessKey = JSON.parse(localStorage.getItem('secretAccessKey') || null);
...
export const getData = async(cognito) => {
    ...
    const request = await cognito.sign(
    `${httpApiURL}/signup/testData`, {
        method: 'GET'
    });
